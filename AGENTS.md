# AGENTS.md — BrickSeeker

Conventions and hard-won context for working on this repo. Read this before making changes.

## What this is

BrickSeeker: SwiftUI iOS app that scans a LEGO set's box text via the camera (or an
...
(`BrickSeeker.xcodeproj/` is gitignored) — run `xcodegen generate` after cloning, and after any
file add/remove or `project.yml` edit. **Never hand-edit `BrickSeeker.xcodeproj`** or the generated
`BrickSeeker/Resources/Info.plist`; edit `project.yml` and regenerate, or your changes get silently
overwritten.

```bash
xcodegen generate   # required after clone; re-run after adding/removing files or editing project.yml
xcodebuild -project BrickSeeker.xcodeproj -scheme BrickSeeker -destination 'generic/platform=iOS Simulator' build
```
...
nothing and wasn't worth the upkeep). Don't add `Tests/`/`BrickSeekerTests` back, or a test step to
...
`HomeView` is the app root (`BrickSeekerApp` toggles between it and `ScannerView` via local
...
`HomeViewModel` is owned by `BrickSeekerApp` (`@State private var homeViewModel`), not by `HomeView`
...
reachability), not "can we reach rebrickable.com specifically". `BrickSeekerApp` shows a small
...
xcconfig indirection **and** (b) not committing the generated `BrickSeeker.xcodeproj` at all (it's
